...
class abstract_loader():
    ...
    def get_image(self, path, additional, sample):
        ...
        if not self.use_cache:
            ...
            if self.apply_augmentations:
                image =  self.composed_all_augmentations(image, additional)
        ...
        image = self.load_input(image_path, additional)
        if self.apply_augmentations:
            image = apply_augmentations_and_cache(image, additional, image_path, all_augmentations,
    ...
    def get_images(self, paths, additionals, sample):
        ...
        images = [self.get_image(path, additional, sample) for path, additional in zip(paths, additionals)]
