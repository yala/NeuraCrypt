...
class Normalize_Tensor_2d_Img_Per_Chan(Abstract_augmentation):
    ...
    def __call__(self, img, additional=None):
        ...
        return img.sub_(self.channel_means[channel]).div_(self.channel_stds[channel])
...
@RegisterTensorAugmentation("frame_dropout")
class FrameDropout(Abstract_augmentation):
    '''
        Drop a slice with some probability (make it black).
    '''
...
class GaussianNoise(Abstract_augmentation):
    ...
    def __call__(self, image, additional=None):
        if additional is not None and 'seed' in additional:
            self.set_seed(additional['seed'])
        return image + torch.randn(image.shape[-2:])*self.sigma + self.mu
...
class Force_Num_Chan_Tensor_2d(Abstract_augmentation):
    ...
    def __call__(self, img, additional=None):
        ...
        if not existing_chan == self.args.num_chan:
            return img.expand(self.args.num_chan, *img.size()[1:])
        return img
