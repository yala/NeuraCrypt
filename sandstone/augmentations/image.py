...
import pdb
from sandstone.augmentations.factory import RegisterImageAugmentation
from sandstone.augmentations.abstract import Abstract_augmentation
import warnings

@RegisterImageAugmentation("scale_2d")
...
@RegisterImageAugmentation("rand_hor_flip")
...
@RegisterImageAugmentation("rand_ver_flip")
...
@RegisterImageAugmentation("random_resize_crop")
class RandomResizedCrop(Abstract_augmentation):
    ...
    def __init__(self, args, kwargs):
        ...
        self.transform = A.RandomResizedCrop(height = h, width = w, scale = (min_scale, max_scale), ratio = (min_ratio, max_ratio))
...
@RegisterImageAugmentation("random_crop")
...
@RegisterImageAugmentation("blur")
...
@RegisterImageAugmentation("center_crop")
...
@RegisterImageAugmentation("color_jitter")
...
@RegisterImageAugmentation("random_hue_satur_val")
class Hue_Saturation_Value(Abstract_augmentation):
    ...
    def __init__(self, args, kwargs):
        ...
        kwargs_len = len(kwargs.keys())
        assert kwargs_len <= 3
        val, satur, hue = int(kwargs['val']) if 'val' in kwargs else 0, int(kwargs['satur']) if 'satur' in kwargs else 0, int(kwargs['hue']) if 'hue' in kwargs else 0
        self.transform = A.HueSaturationValue(hue_shift_limit = hue, sat_shift_limit = satur, val_shift_limit = val, p =1)
...
@RegisterImageAugmentation("random_brightness_contrast")
class Random_Brightness_Contrast(Abstract_augmentation):
    ...
    def __init__(self, args, kwargs):
        ...
        self.transform = A.RandomBrightnessContrast(brightness_limit = brightness,  contrast_limit= contrast, p = 1)
    def __call__(self, img, additional=None):
        if additional is not None and 'seed' in additional:
            self.set_seed(additional['seed'])
        return self.transform(image=img)['image']

@RegisterImageAugmentation("gaussian_blur")
...
@RegisterImageAugmentation("rotate_range")
...
@RegisterImageAugmentation("rotate_90")
...
@RegisterImageAugmentation("align_to_left")
class Align_To_Left(Abstract_augmentation):
    ...
    def __call__(self, img, additional=None):
        raise NotImplementedError("This function must be reimplemented to support albumentations instead of torchvision")
        left = img.copy()
...
@RegisterImageAugmentation("grayscale")
